        ...
        <div x-show="showModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" style="display: none;">
            <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-md bg-white">
                ...
                <form wire:submit.prevent="save">
                    <div class="mb-4">
                        <label for="nombreOcupacion" class="block text-sm font-medium">Nombre Ocupacion</label>
                        <input type="text" wire:model="nombreOcupacion" id="nombreOcupacion" class="mt-1 block w-full" required>
                        @error('nombreOcupacion') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded">
                            @if ($id_ocupacion) Actualizar @else Crear @endif
                        </button>
                    </div>
                </form>
            </div>
