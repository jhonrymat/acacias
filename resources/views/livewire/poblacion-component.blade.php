        ...
        <div x-show="showModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full" style="display: none;">
            <div class="relative top-20 mx-auto p-5 border w-1/2 shadow-lg rounded-md bg-white">
                ...
                <form wire:submit.prevent="save">
                    <div class="mb-4">
                        <label for="nombrePoblacion" class="block text-sm font-medium">Nombre Poblacion</label>
                        <input type="text" wire:model="nombrePoblacion" id="nombrePoblacion" class="mt-1 block w-full" required>
                        @error('nombrePoblacion') <span class="text-red-500">{{ $message }}</span> @enderror
                    </div>
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded">
                            @if ($id_poblacion) Actualizar @else Crear @endif
                        </button>
                    </div>
                </form>
            </div>
