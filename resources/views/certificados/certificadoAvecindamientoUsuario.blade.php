...
<body>
    ...
    <div class="header">
        ...
        <div class="header-image-container">
            <img class="header-image" src="{{ public_path('images/imagen_header_avecindamiento.png') }}"
                alt="Encabezado">
        </div>
    </div>
    ...
    <div class="content" style="text-align: justify;">
        <p>
            De conformidad con la <strong> Resolución Municipal N° 819 del 05 de Septiembre del 2022 </strong> "Por la
            cual se hace una
            delegación de funciones en un Secretario de Despacho" establecido en el Artículo Primero, en el cual delega
            a la Secretaria de Gobierno del Municipio de Acacías Meta o quien haga sus veces conforme a la facultad
            consagrada en el artículo 82 del Código Civil Colombiano y en el Articulo Segundo el Secretario de Gobierno
            expedirá los Certificados de Residencia
        </p>
    </div>
    ...
    <div class="content">
        <p>Que, <strong>{{ $solicitante }}</strong>, identificado(a) con {{ $tipoDocumento }} No
            <strong>{{ $cedula }}</strong> expedida en <strong>{{ $ciudad_expedicion }}</strong>, Quien reside en
            el inmueble ubicado en la
            <strong>{{ $direccion }}</strong>{{ $articulo }} {{ $zona }}
            <strong>{{ $barrio_vereda }}</strong>, {{ $tipo_unidad }} <strong>{{ $codigo_numero }}</strong>.
        </p>
        <p>Que a su vez se pudo constatar, en la visita realizada en la fecha {{ $fecha_visita }}.
            a través del tramite N° {{ $id }}; por los Técnicos Operativos del Área de
            Control Físico adscritos a la Secretaría de Gobierno Municipal.
        </p>
    </div>
