...
class SolicitudAvecindamientoDatatable extends DataTableComponent
{
    ...
    public function AceptarTodas()
    {
        ...
        try {
            foreach ($this->selectedRows as $solicitudId) {
                // Buscar la solicitud
                $solicitud = SolicitudAvecindamiento::find($solicitudId);
                if (!$solicitud) {
    ...
    public function rechazarStatus()
    {
        ...
        $this->dispatch(
            ...
            icon: 'info',
            title: '¿Estás seguro?',
            text: 'No se completaran estas solicitudes'
        );
    ...
    public function RechazarTodas()
    {
        // Asegurar que hay filas seleccionadas antes de proceder
        if (count($this->selectedRows) === 0) {
            $this->dispatch('sweet-alert-good', icon: 'warning', title: 'Advertencia', text: 'Debe seleccionar al menos una fila para pasarlas a No completadas.');
            return;
        }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make("persona a cargo", "nombre_persona_cargo")
                ...
                ->searchable()
                ->collapseAlways(),
            Column::make("Doumento de persona", "documento_persona_cargo")
                ->sortable()
                ->searchable()
                ->collapseAlways(),
